...
- **Layout persistence** — Broadcaster layout is saved to localStorage and restored on reload: sound pads, mixer strip settings (including per-channel monitor states), effects state/parameters, quality mode, and selected input device (matched by name when the browser rotates device IDs)
...
- **Setlist** — Plan a show off-air: the track search queues tracks with full Deezer metadata into a setlist (persisted per browser), and "Mark played" publishes each one to the live track list the moment it airs
- **Auto-identify**: automatic song identification using AcoustID/Chromaprint (shown when `ACOUSTID_API_KEY` is configured). Strictly per-session consent: off by default, enabled via a checkbox in the broadcast settings dialog or the ear icon mid-show, always reset when the broadcast ends. Matches are enriched with ISRC/album/label via Deezer for reporting
- **Show report** — When a broadcast ends, a report shows the listener curve, peak, average, time on air, and track count, with a per-minute listener CSV export
- **SoundExchange compliance** — Owner-only Admin tab that builds quarterly Reports of Use (artist, title, ISRC, album, label, performances) from persisted tracks and a listener session log (WebRTC + relay listeners), computes aggregate tuning hours, flags tracks missing identification with a two-click Deezer fix-up, and exports ROU and playlist files
...
- **Custom receive URLs** — Your receive URL defaults to your username (e.g. `/receive/dj-nova` for `DJ_Nova`), and you can set a custom slug instead (e.g. `/receive/elpasorocks` or `/receive/farmers-market`). Lowercase letters, numbers, and hyphens, 2–40 characters. Usernames are reserved: a custom slug that matches another user's username is rejected, and new usernames that would share a URL with an existing user are refused. Previously used slugs are stored server-side and shown as suggestions with live/available status indicators. Slugs can be freely reused across broadcasts (blocked only while a room with that slug is live)
...
- **Stream URL sharing** — Every broadcast exposes a Stream URL on the receiver page — both integration-based (Icecast/Shoutcast) and a built-in HTTP relay. The relay serves MP3 via server-side FFmpeg transcoding (WebM→MP3) with Icecast-compatible ICY headers, so the URL works in RadioDJ, VLC, internet-radio.com, and any media player that accepts standard HTTP audio streams. The MP3 output is frame-aligned and paced: a stall on the broadcaster's link is filled with silent frames at the real-time rate (players never see the byte stream pause, which is what makes RadioDJ drop it), new players get the last four seconds on connect (burst-on-connect), and if the broadcaster disconnects the silence continues for up to 10 minutes while waiting for them to return. Relay health (feeding / stalled) is shown in the console's Stats panel. The relay is hardened for long-running streams with error-resilient writers, FFmpeg lifecycle safety, and proxy buffering bypass
- **Broadcast recovery** — A dropped signaling connection heals itself: the console rejoins the room and restarts the stream relay on reconnect. If the broadcaster closes the browser, reopening the page detects the previous broadcast and prompts to resume it (the owner's resume takes over even if the server still holds the old session). The stream stays alive (silence keepalive) so media player listeners don't have to reconnect
...
3. Optionally set a **custom receive URL** (e.g. `elpasorocks`) in the Receive URL panel. Leave it blank to use your username
